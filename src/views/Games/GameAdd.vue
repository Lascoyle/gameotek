<template>
    <div id="game-add" class="p-6 lg:p-8 lg:pt-20 h-full">
        ...
        <form @submit.prevent="addGame" class="w-8/12 mx-16 p-10 bg-white rounded-lg">
            ...
            <div class="game-add-name py-3 text-gray-700 font-semibold">
                <label for="game-add-name" name="game-add-name" class="block py-2">Game Title :</label>
                <input type="text" disabled="disabled" :value="gameAddForm.title = game.name" class="block rounded-full bg-gray-100 text-gray-500 px-4 py-2">
            </div>
            <div class="game-add-released py-3 text-gray-700 font-semibold">
                <label for="game-add-released" name="game-add-released" class="block py-2">Game Released :</label>
                <input type="text" disabled="disabled" :value="gameAddForm.released_date = formatDate(game.released)" class="block rounded-full bg-gray-100 text-gray-500 px-4 py-2">
            </div>
            <div class="game-add-image py-3 text-gray-700 font-semibold">
                <label for="game-add-image" name="game-add-image" class="block py-2">Game Image :</label>
                <input type="text" disabled="disabled" :value="gameAddForm.image = game.background_image" class="block rounded-full bg-gray-100 text-gray-500 px-4 py-2">
            </div>
